...
interface FontColorToolbarButtonProps extends DropdownMenuProps {
  nodeType: string;
  tooltip?: string;
}

export function FontColorToolbarButton({ children, nodeType, tooltip }: FontColorToolbarButtonProps) {
  const editorRef = useEditorRef()

  const selectionDefined = useEditorSelector(
    editorRef => !!editorRef.selection,
    []
  )

  const color = useEditorSelector(
    (editorRef) => editorRef.api.mark(nodeType) as string,
    [nodeType]
  )
  // 选中的颜色
  ...
  const updateColor = React.useCallback(
    (value: string) => {
      if (editorRef.selection) {
        ...
        editorRef.tf.select(editorRef.selection)
        editorRef.tf.focus()

        editorRef.tf.addMarks({ [nodeType]: value })
      }
    },
    [editorRef, nodeType]
  )
  ...
  const clearColor = React.useCallback(() => {
    if (editorRef.selection) {
      ...
      if (selectedColor) {
        editorRef.tf.removeMarks(nodeType)
      }

      onToggle()
    }
  }, [editorRef, selectedColor, onToggle, nodeType])
...
interface ColorCustomProps extends React.ComponentProps<'div'> {
  colors: TColor[];
  customColors: TColor[];
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <input
        {...props}
        ref={useComposedRef(
          typeof props.ref === 'string' ? undefined : props.ref,
          inputRef
        )}
...
interface ColorDropdownMenuItemProps extends DropdownMenuItemProps {
  isBrightColor: boolean;
  isSelected: boolean;
  value: string;
  updateColor: (color: string) => void;
...
interface ColorDropdownMenuItemsProps extends React.ComponentProps<'div'> {
  colors: TColor[];
  updateColor: (color: string) => void;
  color?: string;
}
