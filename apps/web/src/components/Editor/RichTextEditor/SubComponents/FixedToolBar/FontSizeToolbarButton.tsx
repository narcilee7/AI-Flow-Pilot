...
export function FontSizeToolbarButton() {
  ...
  const cursorFontSize = useEditorSelector(
    (editor) => 
    {
    ...
    const handleInputChange = () => {
      ...
      if (Number.parseInt(newSize) < 0 || Number.parseInt(newSize) > 100) {
        // newSize 不在 0-100 之间，则不进行设置
        editor.tf.focus()
        return 
      }
      // 如果 newSize 和 cursorFontSize 不一致，则设置 fontSize 标记
      ...
      <div className='flex h-7 items-center gap-1 rounded-md bg-muted/60 p-0'>
        ...
        <Popover open={isFocused} modal={false}>
          <PopoverTrigger asChild>
            <input 
              className='h-full w-10 shrink-0 bg-transparent text-center px-1 text-sm hover:bg-muted'
              value={displayValue}
              onFocus={() => {
                setIsFocused(true)
